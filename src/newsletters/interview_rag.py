...
*generated by Claude 3.5 Sonnet, modified by Claude 3.5 Sonnet (removed logfire integration)
...
async def database_connect(create_db: bool = False) -> AsyncGenerator[asyncpg.Pool, None]:
    ...
    server_dsn = os.environ['DATABASE_URL']
    ...
    pool = await asyncpg.create_pool(f'{server_dsn}')
    try:
        yield pool
    finally:
        await pool.close()
